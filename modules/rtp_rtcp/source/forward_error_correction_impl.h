...
#ifndef MODULES_RTP_RTCP_SOURCE_FORWARD_ERROR_CORRECTION_IMPL_H_
#define MODULES_RTP_RTCP_SOURCE_FORWARD_ERROR_CORRECTION_IMPL_H_
...
class ForwardErrorCorrectionImpl {
 public:
  ...
  class ProtectedPacket : public SortablePacket {
   public:
    ...
    rtc::scoped_refptr<ForwardErrorCorrectionImpl::Packet> pkt;
  };
  ...
  ~ForwardErrorCorrectionImpl();

  // Creates a ForwardErrorCorrectionImpl tailored for a specific FEC scheme.
  static std::unique_ptr<ForwardErrorCorrectionImpl> CreateUlpfec(uint32_t ssrc);
  static std::unique_ptr<ForwardErrorCorrectionImpl> CreateFlexfec(
      uint32_t ssrc,
 ...
 private:
  ForwardErrorCorrectionImpl(std::unique_ptr<FecHeaderReader> fec_header_reader,
...
class FecHeaderReader {
 public:
  ...
  // to ForwardErrorCorrectionImpl::EncodeFec().
  ...
  virtual bool ReadFecHeader(
      ForwardErrorCorrectionImpl::ReceivedFecPacket* fec_packet) const = 0;
...
class FecHeaderWriter {
 public:
  ...
  virtual void FinalizeFecHeader(
      ...
      size_t packet_mask_size,
      ForwardErrorCorrectionImpl::Packet* fec_packet) const = 0;
